...
```
*Lubridate* is used to convert the UTCdate to decimal hours.  Analytic data are processed such that the plot shows the mean step-count for a given interval averaged across the 53-day data set from within the 61-day sampling period.



## Imputing missing values
Recall, eight days for which no data are exluded from the analyses reported above.  Too, excluded are missing data from within a day for which valid step-count data exists. There are technical references that provide guidance for imputing missing values, e.g. [Missing-data imputation](http://www.stat.columbia.edu/~gelman/arm/missing.pdf).  For this exercise, a more pedestrian approach is used to impute missing data while reducing bias.
